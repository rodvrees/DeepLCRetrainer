...
def retrain(
    ...
    for dataset_name in datasets:
        ...
        df = cnn_functions.get_feat_df(
            df, costum_modification_file=costum_modification_file
        )

        if use_correction_factor:
            correction_factor = (df["tr"].max()) / 10.0
        else:
...
@Gooey(
    program_name="DeepLC re-tR-ainer", default_size=(720, 790), monospace_display=True
)
def main():
